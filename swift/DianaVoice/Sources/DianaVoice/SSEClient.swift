...
final class SSEClient: ObservableObject {
    ...
    private var runtimeMood: AvatarMood = .idle
    private var pttMood: AvatarMood?

    /// An older transcription or runtime idle event must not hide a new PTT
    /// recording. Restore the latest runtime state when local work finishes.
    func setPttMood(_ mood: AvatarMood?) {
        pttMood = mood
        self.mood = Self.effectiveMood(runtime: runtimeMood, ptt: pttMood)
    }

    static func effectiveMood(runtime: AvatarMood, ptt: AvatarMood?) -> AvatarMood {
        // A hot microphone always wins over an older pending transcription,
        // regardless of whether PTT or voice_listen owns that recording.
        if runtime == .listening || ptt == .listening { return .listening }
        return ptt ?? runtime
    }
    ...
    private func dispatch(event: String, data: String) {
        switch event {
        case "voice-state":
            runtimeMood = AvatarMood.parse(data)
            mood = Self.effectiveMood(runtime: runtimeMood, ptt: pttMood)
